		...
		if (age > 18) {
			if (customer_Dao.check1(email).isEmpty() && customer_Dao.check2(mob).isEmpty()) {
				...
				if(customer2.getGender().equals("female")) {
					...
					resp.getWriter().print("<h1 style='color:white;'> Hello sir</h1>");
				}
				resp.getWriter().print("<h1 style='color:white;'> Account has been created successfully.</h1>");
				req.getRequestDispatcher("customerLogin.html").include(req, resp);
			} 
			else {
				resp.getWriter().print("<h1 style='color:white;'> Account is already existed.</h1>");
			}
		} 
		else {
			resp.getWriter().print("<h1 style='color:white;'> You are not eligible to create an account.</h1>");
		}
	}
}
