...
public class CustomerLogin extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (customer == null) {
			resp.getWriter().print("<h1 style='color:white;'>Invalid customer id</h1>");
			req.getRequestDispatcher("Home.html").include(req, resp);
		} 
		else {
			if (customer.getPwd().equals(password)) {
				resp.getWriter().print("<h1 style='color:white;'>Login Successfull</h1>");
				//session tracking
				...
				resp.getWriter().print("<h1 style='color:white;'>Invalid Password</h1>");
				req.getRequestDispatcher("Home.html").include(req, resp);
			}
		}
